...
Ext.namespace('ScheduleProposeAndApprove');                       //定义命名空间
	...
	/* ************* 定义全局变量开始 ************* */
    ScheduleProposeAndApprove.searchParams = {status : STATUS_XJ}; ;
...
ScheduleProposeAndApprove.grid = new Ext.yunda.Grid({
    ...
    saveURL: ctx + '/scheduleProposeAndApprove!saveOrUpdate.action',             //保存数据的请求URL
    saveFormColNum:1,
    tbar: ['add',  '-', {text: '发送申请', iconCls: 'messageIcon', handler: function() {
				...
				for (var i = 0; i<items.length; i ++) {
					if (STATUS_DSP === items[i].data.status || STATUS_YSP == items[i].data.status) {
						MyExt.Msg.alert('只能发送新建的命令单！');
						return;
					}
					ids.push(items[i].id);
				}
				Ext.Msg.confirm('提示', '是否发送所有选择申请，是否确认继续？？', function(btn) {
					if ('yes' == btn) {
						$yd.request({
		        			url: ctx + '/scheduleProposeAndApprove!sendApplication.action',
		        			params: {
		        				ids: Ext.encode(ids)
		        			}
		        		}, function () {
    	   ...
    	   handler: function(){
				...
				var ids = [];
				for (var i = 0; i<items.length; i ++) {
					if (STATUS_XJ === items[i].data.status || STATUS_YSP == items[i].data.status) {
						MyExt.Msg.alert('只能撤销待审批的命令单！');
						return;
					}
					ids.push(items[i].id);
				}
				Ext.Msg.confirm('提示', '是否撤销所选命令单，是否确认继续？？', function(btn) {
					if ('yes' == btn) {
						$yd.request({
		        			url: ctx + '/scheduleProposeAndApprove!rovokeApplication.action',
		        			params: {
		        				ids: Ext.encode(ids)
		        			}
		        		}, function () {
    	...
    	xtype:"radio", name:"status", boxLabel:'新建'+"&nbsp;&nbsp;&nbsp;&nbsp;", checked:true,
    ...
    {   
    	xtype:"radio", name:"status", boxLabel:'待审批',
    	handler: function(radio, checked){
    		if(checked){
	...
	fields: [{
		...
		header:'主题', dataIndex:'topic', editor: {maxLength: 100}
	},{
		header:'命令号', dataIndex:'orderNum', editor:{ maxLength: 20}
	},{
		...
		header:'内容', dataIndex:'content',	width: 200, editor:{xtype: 'textarea', maxLength: 200}
	},{
		header:'申请时间', dataIndex:'proposeTime', xtype:'datecolumn', format: 'Y-m-d h:m', editor:{ xtype:'my97date', format: 'Y-m-d H:i', hidden:true }
	},{
		header:'审批时间', dataIndex:'approveTime', hidden:true, editor:{xtype:'hidden'}
	},{
		header:'审批意见', dataIndex:'approvalOpinion', editor:{xtype: 'hidden'}
	},{
		header:'命令单状态', dataIndex:'status', editor:{xtype: 'hidden'}, 
		renderer: function(v, m, r) {
			if (STATUS_XJ === r.get('status') || STATUS_DSP === r.get('status')) {
	   ...
	   toEditFn: function(grid, rowIndex, e){
	   	...
	   	if (record && STATUS_DSP == record.get('status')) {
	   		MyExt.Msg.alert('待审批状态的命令单不能再修改！');
			return;
	   	}
	   	if (record && STATUS_YSP == record.get('status')) {
	   		MyExt.Msg.alert('已审批状态的命令单不能再修改！');
			return;
	   	}
