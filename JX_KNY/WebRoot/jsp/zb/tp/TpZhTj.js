Ext.onReady(function(){
	...
	TpZhTj.searchForm = new Ext.form.FormPanel({
		...
		buttons:[{
	            ...
	            text: "EXCEL", iconCls: "resetIcon", handler: function(){ 
	            	var url = ctx + "/zbglTp!exportTpListByParm.action";
	            	var params = [];
	                var fields = getFields("group_by_fields",2);
	                var grid = TpZhTj.grid;
	                var deteilgrid = TpZhTj.deteilgrid;
	                var form = TpZhTj.searchForm.getForm();	
			        var searchParam = form.getValues();
			        searchParam = MyJson.deleteBlankProp(searchParam);
			        //是否有明细表，如果有ture 没有 false
			        searchParam.flag = false;
	                
	                //判断是否有明细数据，如果有明细数据的话，那么查询条件以明细中的trainNo,professionalTypeName,faultFixFullName,faultName字段作为条件
	            	//如果没有明细数据，那么查询条件以查询表单的字段为条件getCount( ) 
	            	var num = deteilgrid.store.getCount();
	            	if(num){
	            		var recordVal  = deteilgrid.store.getAt(0);
	                ...
	                params.push(getPatterns(deteilgrid,"deteailpatterns"));
	...
	TpZhTj.grid = new Ext.yunda.Grid({
	    loadURL: ctx + '/zbglTp!findTpPageListByParm.action',//装载列表数据的请求URL
	    page : false,
	    storeAutoLoad: false,
	    singleSelect: true,
	    viewConfig: null,
	...
	TpZhTj.deteilgrid = new Ext.yunda.Grid({
	    loadURL: ctx + '/zbglTp!findTpDeteailPageListByParm.action',//装载列表数据的请求URL
	    page : false,
	    singleSelect: true,
	    viewConfig: null,
	    tbar:[],
		...
		fields: [
			...
			{header:'施修人', dataIndex:'handlePersonId'},
			{header:'处理结果', dataIndex:'repairResult',renderer: function(value, metaData, record, rowIndex, colIndex, store){
