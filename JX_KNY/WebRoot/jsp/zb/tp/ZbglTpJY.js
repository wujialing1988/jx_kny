...
Ext.namespace('ZbglTpJY');                       //定义命名空间
ZbglTpJY.searchParam = {};
ZbglTpJY.labelWidth = 90;
ZbglTpJY.fieldWidth = 150;
ZbglTpJY.saveForm = new Ext.form.FormPanel({
	baseCls: "x-plain", align: "center",	defaultType: "textfield",	defaults: { anchor: "98%" },
    layout: "form",		border: false, frame: true, labelWidth: ZbglTpJY.labelWidth,
    items: [{
        xtype: "panel",	border: false,	baseCls: "x-plain",	layout: "column",	align: "center", 
        items: [
        {
            baseCls:"x-plain", align:"center", layout:"form", defaultType:"textfield", labelWidth:ZbglTpJY.labelWidth,
            columnWidth: 0.5, 
            items: [
        		{ fieldLabel:"放行人", name:"handlePersonName", value: empname, width:ZbglTpJY.fieldWidth, style:"border:none;background:none;", readOnly:true},
            ...
            columnWidth: 0.5, 
            items: [
            	{ fieldLabel:"放行时间", name:"handleTime", xtype: 'my97date', my97cfg: {dateFmt:'yyyy-MM-dd HH:mm:ss'}, width: ZbglTpJY.fieldWidth, allowBlank: false}
            ]
        }]
    },{    	
        ...
        items: [{
            ...
            labelWidth: ZbglTpJY.labelWidth,      columnWidth: 1, 
            items: [{ xtype: "textarea", fieldLabel:"放行原因", name:"exceptionReason", width:ZbglTpJY.labelWidth + ZbglTpJY.fieldWidth + 8, maxLength: 300}]
       }]
    }]
});
ZbglTpJY.saveFn = function() {
    var form = ZbglTpJY.saveForm.getForm(); 
    if (!form.isValid()) return;
    var data = form.getValues();   
    
    var datas = ZbglTpJY.grid.selModel.getSelections();
    if (datas.length < 1) {
    ...
    if(ZbglTpJY.grid.loadMask)   ZbglTpJY.grid.loadMask.show();
    var cfg = {
        scope: this, url: ctx + '/zbglTpException!saveForLwfx.action', 
        params: params,
        success: function(response, options){
            if(ZbglTpJY.grid.loadMask)   ZbglTpJY.grid.loadMask.hide();
            var result = Ext.util.JSON.decode(response.responseText);
            if (result.success == true) {
            	ZbglTpJY.grid.store.load();
            	ZbglRdpJYWin.grid.store.load();
            	ZbglTpJY.saveWin.hide();
            	alertSuccess();                
            } else {
...
ZbglTpJY.saveWin = new Ext.Window({
    title:"放行原因填写", width: (ZbglTpJY.labelWidth + ZbglTpJY.fieldWidth + 8) * 2 + 60, 
    plain:true, closeAction:"hide", buttonAlign:'center', maximizable:true, items:ZbglTpJY.saveForm, 
    buttons: [{
        text: "确认", iconCls: "saveIcon", handler: ZbglTpJY.saveFn
    }, {
        text: "关闭", iconCls: "closeIcon", handler: function(){ ZbglTpJY.saveWin.hide(); }
    }]
});

ZbglTpJY.grid = new Ext.yunda.Grid({
    loadURL: ctx + '/zbglTp!pageQuery.action',                 //装载列表数据的请求URL
    viewConfig: null,
    storeAutoLoad: false,
    tbar:[
//    	{
//    	text:'例外放行', iconCls:'addIcon', handler:function(){
//    		var grid = ZbglTpJY.grid;
//    		if(!$yd.isSelectedRecord(grid)) return;
//    		var datas = grid.selModel.getSelections();
//    		for (var i = 0; i < datas.length; i++) {
//    			if (datas[i].get("faultNoticeStatus") != STATUS_DRAFT) {
//    				MyExt.Msg.alert("只有未处理的提票活才能例外放行，请重新选择");
//	        		return;
//    			}
//    		}
//    		ZbglTpJY.saveWin.show();
//    	}
//    }, 
    '-', {
    ...
    searchFn: function(searchParam){
    	ZbglTpJY.searchParam = searchParam;
    	this.store.load();
    }
});
ZbglTpJY.faultNoticeStatus = STATUS_DRAFT + "," + STATUS_OPEN + "," + STATUS_OVER + "," + STATUS_CHECK;
ZbglTpJY.checkQuery = function() {
	ZbglTpJY.faultNoticeStatus = "-1";
	var checkBoxGroup = ZbglTpJY.grid.getTopToolbar().findByType("checkbox");
	for(var i = 0; i < checkBoxGroup.length; i++) {
		if(checkBoxGroup[i].checked) {
			ZbglTpJY.faultNoticeStatus = ZbglTpJY.faultNoticeStatus + "," + checkBoxGroup[i].inputValue;
		}
	}
	ZbglTpJY.grid.store.load();
}
ZbglTpJY.grid.store.on("beforeload", function(){	
	var searchParam = ZbglTpJY.searchParam;
	var whereList = [] ;
	...
	whereList.push({propName:'faultNoticeStatus', propValues: ZbglTpJY.faultNoticeStatus.split(",") ,compare : Condition.IN}) ;
