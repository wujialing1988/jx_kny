...
ZbglHoCase.grid = new Ext.yunda.Grid({
    ...
    tbar:[{
    	text: '详情查看', iconCls:'messageIcon', handler:function(){
			...
			// 设置接车时间的显示值
			if(record.get('arrivedTime') != null) ZbglHoCaseItem.caseForm.find('name', 'arrivedTime')[0].setValue(record.get('arrivedTime').format('Y-m-d H:m'));
	...
	fields: [{
		...
		header:'迎检时间', dataIndex:'arrivedTime', xtype:'datecolumn',format: "Y-m-d H:i", editor:{ xtype:'my97date' }
	},{
		...
		header:'迎检人ID', dataIndex:'toPersonId', hidden:true, editor:{ xtype:'numberfield', maxLength:18 }
	},{
		header:'迎检人', dataIndex:'toPersonName', editor:{  maxLength:50 }
	},{
