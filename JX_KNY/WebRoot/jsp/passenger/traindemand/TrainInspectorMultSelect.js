...
Ext.namespace("TrainInspectorMultSelect");
TrainInspectorMultSelect.routingIdx = '';
TrainInspectorMultSelect.runningDate = '';
TrainInspectorMultSelect.baseParams = {};
// 人员列表数据容器
TrainInspectorMultSelect.userstore = new Ext.data.JsonStore({
	url : ctx + "/trainInspector!findInspectorList.action",
	autoLoad : false,
	root : "root",
	storeId:"idx",
	remoteSort : true,
	totalProperty : "totalProperty",
	fields : ["empid", "empcode", "empname", "gender","orgname","orgid","workHours"]
});
/** ****************************************************************************************** */
//机构选择树
TrainInspectorMultSelect.OrgTree = Ext.extend(Ext.tree.TreePanel, {
	parentObj:null,
	constructor : function() {
		TrainInspectorMultSelect.OrgTree.superclass.constructor.call(this, {
			...
			listeners : {
				click : function(node, e) {
					...
					TrainInspectorMultSelect.baseParams.orgid =  node.id;
					this.parentObj.grid.store.baseParams.orgcode = node.attributes["orgcode"];
					TrainInspectorMultSelect.baseParams.orgseq = node.attributes["orgseq"];
	...
	//查询前添加过滤条件
TrainInspectorMultSelect.userstore.on('beforeload' , function(){
		TrainInspectorMultSelect.baseParams.routingIdx = TrainInspectorMultSelect.routingIdx;
		TrainInspectorMultSelect.baseParams.runningDate = TrainInspectorMultSelect.runningDate;
		this.baseParams.entityJson = Ext.util.JSON.encode(TrainInspectorMultSelect.baseParams)
});	
/** ****************************************************************************************** */

//分页工具
TrainInspectorMultSelect.pagingToolbar = Ext.yunda.createPagingToolbar({store: TrainInspectorMultSelect.userstore});
//选择模式，勾选框可多选
TrainInspectorMultSelect.sm = new Ext.grid.CheckboxSelectionModel({singleSelect:false});
//人员列表
TrainInspectorMultSelect.UserList = Ext.extend(Ext.grid.GridPanel, {
	parentObj:null,
	constructor : function() {
		TrainInspectorMultSelect.UserList.superclass.constructor.call(this, {
			viewConfig : {
				forceFit : true
			},
		...
		//                	ids = "'"+ids+"'";
			...
			colModel : new Ext.grid.ColumnModel([
				...
				new Ext.grid.RowNumberer(), {
					...
					renderer: function(value, metaData, record, rowIndex, colIndex, store){
						...
						return value + "("+(Ext.isEmpty(workHours)? "0": formatTimeForHours(workHours, 'm')) + ")";
					}
				}, {
					sortable : true,
					header : "人员代码",
			...
			store : TrainInspectorMultSelect.userstore,
			bbar: TrainInspectorMultSelect.pagingToolbar,
			listeners:{
				//双击选择人员
//				"rowdblclick": {
...
/** ****************************************************************************************** */
//弹出窗口
TrainInspectorMultSelect.UserSelectWin = Ext.extend(Ext.Window, {
	tree : new TrainInspectorMultSelect.OrgTree(),
	grid : new TrainInspectorMultSelect.UserList(),
	parentObj:null,
	modal:true,
    beforeShow : function(){
...
/** ****************************************************************************************** */
TrainInspectorMultSelect.UserSelect = Ext.extend(Ext.form.TriggerField, {
	valueField : 'empid',
	displayField : 'empname',
	hiddenName : 'userselect_id',
	editable :true,			
	win : new TrainInspectorMultSelect.UserSelectWin(),			
	returnField:[],
...
//			this.win.show();
	...
	initComponent : function() {
		TrainInspectorMultSelect.UserSelect.superclass.initComponent.call(this);
	},
	...
	onRender : function(ct, position) {				
		...
		TrainInspectorMultSelect.UserSelect.superclass.onRender.call(this, ct, position);        
		if (this.hiddenName) {
	...
	initValue : function() {
		TrainInspectorMultSelect.UserSelect.superclass.initValue.call(this);
		if (this.hiddenField) {
	...
	constructor : function(options) {
		...
		TrainInspectorMultSelect.UserSelect.superclass.constructor.call(this);
	...
	getName : function() {
		...
		return hf && hf.name ? hf.name : this.hiddenName
				|| TrainInspectorMultSelect.UserSelect.superclass.getName.call(this);
	},			
	//获取返回值			
	getValue : function() {				
	...
	setValue : function(r) {				
		...
		{					
			...
			TrainInspectorMultSelect.UserSelect.superclass.setValue.call(this, r.get(this.displayField));
...
Ext.reg('TrainInspector_MultSelectWin', TrainInspectorMultSelect.UserSelect);