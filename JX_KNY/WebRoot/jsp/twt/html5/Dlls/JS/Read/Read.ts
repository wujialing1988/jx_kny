...
class Read
{
    ReadCanvas(FileName: string, cc: SVGSVGElement, Canvas: Canvas)
    {
        var TrackList = new Dictionary<string, TrackLine>();
        ...
        if (FileName != "")
        {
            ...
            try 
            {
                ...
                xmlDoc.find("TextFrame").each((i, j) =>
                {
                    var text = j;
                ...
                var TrackLineList = new Array<TrackLine>();
                xmlDoc.find("TrackLine").each((i, j) =>
                {
                    var text = j;
                ...
                var SwitchesList = new Array<Switches>();
                xmlDoc.find("Switches").each((i, j) =>
                {
                    var text = j;
                ...
                var PositionList = new Array<PositionClass>();
                xmlDoc.find("Position").each((i, j) =>
                {
                    var text = j;
                ...
                xmlDoc.find("Insulation").each((i, j) =>
                {
                    var text = j;
                ...
                var LightList = new Array<Light>();
                xmlDoc.find("Light").each((i, j) =>
                {
                    var text = j;
                    var Track = text.attributes["图形附加的轨道"].value;
                    if (TrackList.Count > 0)
