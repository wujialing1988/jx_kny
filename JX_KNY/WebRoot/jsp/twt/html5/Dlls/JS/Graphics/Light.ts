...
{ 
    ...
    TrackLine: TrackLine;
    ///是否显示双信号灯
    ShowTwoLights: Boolean;
    /// 中转点的集合
    convertPointList: Array<SVGPoint>;
    /// 信号灯上下方位
    UpDown: UpDown;
    ///信号灯左右方位
    LeftRight: LeftRight;
    ...
    S: Svg;
    constructor(StartPoint: string, EndPoint: string, TrackLine: TrackLine)
    {
    ...
    GraphicsRead(Light: Element, TrackLight: Light)
    {
        ...
        TrackLight.LightColor = <LightColor>Light.attributes["图形的颜色"].value;
        TrackLight.UpDown = <UpDown>Light.attributes["图形上下方位"].value;
        TrackLight.HighLow = <HighLow>Light.attributes["图形高低方位"].value;
        TrackLight.LeftRight = <LeftRight>Light.attributes["图形左右方位"].value;
        TrackLight.ShowTwoLights = <Boolean>Light.attributes["图形是否双型号灯"].value;
