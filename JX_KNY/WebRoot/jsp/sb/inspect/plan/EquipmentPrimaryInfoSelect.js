Ext.onReady(function() {
	...
	EquipmentPrimaryInfoSelect.grid = new Ext.yunda.Grid({
		...
		viewConfig : false,
		tbar : ['-', {
			xtype: 'textfield', width: 180, emptyText: '设备名称或编号快速检索', enableKeyEvents: true, listeners: {
				// 输入完成后按回车键查询
				keyup: function( me, e ) {
					if (e.ENTER == e.keyCode) {
						EquipmentPrimaryInfoSelect.grid.store.load();
		...
		toEditFn : function(grid, rowIndex, e) {
			...
			// 保存巡检目录明细
			InspectPlanEquipment.saveFn(InspectPlanEquipment.planIdx, [record.id], function() {
	...
	EquipmentPrimaryInfoSelect.grid.store.on('beforeload', function() {
		...
		// 过滤掉已被添加的设备信息
		whereList.push({
			compare : Condition.SQL, sql : "IDX NOT IN (SELECT EQUIPMENT_IDX FROM E_INSPECT_PLAN_EQUIPMENT WHERE RECORD_STATUS = '0' AND PLAN_IDX ='" + InspectPlanEquipment.planIdx +"')"
		});
		// 只查询“新购”和“调入”的设备
...
//		width : 900, height : 550,
		...
		buttons : [{
			text : '添加', iconCls : 'addIcon', handler : function() {
				...
				// 保存巡检目录明细
				InspectPlanEquipment.saveFn(InspectPlanEquipment.planIdx, equipmentIds, function() {
					// 重新加载设备信息列表，过滤掉已被添加的设备信息
					EquipmentPrimaryInfoSelect.grid.store.reload();
				});
			}
		...
		listeners : {
			...
			beforeshow : function() {
				...
				// 周检默认为A类设备
				if (PERIOD_TYPE_ZJ_CH == periodType) {
					manageClassField.setValue('A');
				// 月检默认为B类设备
				} else if (PERIOD_TYPE_YJ_CH == periodType) {
					manageClassField.setValue('B');
				// 季检默认为C类设备
				} else if (PERIOD_TYPE_JJ_CH == periodType) {
