Ext.onReady(function(){
    Ext.namespace("PartsAccountForExportRegister");
    PartsAccountForExportRegister.searchParam={};
    //PartsAccountForExportRegister.idx="-1111";
    PartsAccountForExportRegister.specificationModel = "";//规格型号
    
    PartsAccountForExportRegister.grid = new Ext.yunda.Grid({
	    ...
	    tbar:['配件名称(规格型号)：',{xtype:'textfield',width:150,id:'partsAcId',disabled:true},'&nbsp;&nbsp;',
	              {text:'确定',iconCls:'yesIcon',handler:function(){
	                   var records=PartsAccountForExportRegister.grid.selModel.getSelections();
	                   if(records.length<1){
	                   	...
	                   	var count=PartsExportRegisterDetail.grid.store.getCount();
       	          		...
       	          		if(count!=0){
       	          			for(var i=0;i<count;i++){
       	          				var record1=PartsExportRegisterDetail.grid.store.getAt(i);
	                   ...
	                   PartsAccountForExportRegister.batchWin.hide();
	                   
	              }},'&nbsp;&nbsp;',
	              {text:'取消',iconCls:'deleteIcon',handler:function(){
	              	    var searchParam={};
				        PartsAccountForExportRegister.batchForm.getForm().reset();
				     	PartsAccountForExportRegister.grid.searchFn(searchParam);
				     	PartsAccountForExportRegister.batchWin.hide();
	              }}
	    ],
	    fields: [  {
	           ...
	           searchFn: function(searchParam){
	           	    searchParam.specificationModel=PartsAccountForExportRegister.specificationModel;
	                PartsAccountForExportRegister.searchParam=searchParam;
	           		this.store.load();	
               }
	});
	PartsAccountForExportRegister.batchForm=new Ext.form.FormPanel({
				...
				},{id:"unloadRepairClass",xtype:"hidden", name:"unloadRepairClass"},
				{
					...
					items:[{
					   ...
					   {xtype:'button',text:'查询',iconCls:'searchIcon',handler:function(){
						   	PartsAccountForExportRegister.searchParam=PartsAccountForExportRegister.batchForm.getForm().getValues();
					     	PartsAccountForExportRegister.grid.searchFn(PartsAccountForExportRegister.searchParam);
				     }}]
				},{
					   columnWidth:.7,
					   frame:true,baseCls:'x-plain',
					   items:[
					     {xtype:'button',text:'重置',iconCls:'resetIcon',handler:function(){
					        var searchParam={};
					         ...
					         PartsAccountForExportRegister.specificationModel = "";
...
PartsAccountForExportRegister.batchWin=new Ext.Window({
     title:'配件选择', width:1000, height:600, plain:true, closeAction:"hide", buttonAlign:'center', layout:'border',frame:true,
    	...
    	items:[{
    	   ...
    	   items:[{
    	      ...
    	      height:100,
    	      title:'查询',
    	      items:[PartsAccountForExportRegister.batchForm]
    	   },{
    	      region:'center',
    	      frame:true,
    	      layout:'fit',
    	      items:[PartsAccountForExportRegister.grid]
    	   }]
    	}]
	});
   //树形面板选择按钮
   jx.pjwz.partbase.component.returnFn = function(node){
     ...
     if(node.id == 'ROOT_0'){
   		 Ext.getCmp("partsAcId").setValue("");
	     PartsAccountForExportRegister.searchParam.specificationModel="";
	     PartsAccountForExportRegister.specificationModel = "";
   		}else {
   		Ext.getCmp("partsAcId").setValue(node.text);
        PartsAccountForExportRegister.searchParam.specificationModel=node.attributes.specificationModel;
        PartsAccountForExportRegister.specificationModel = node.attributes.specificationModel;
   	}
     PartsAccountForExportRegister.searchParam=MyJson.deleteBlankProp(PartsAccountForExportRegister.searchParam);
	 PartsAccountForExportRegister.grid.searchFn(PartsAccountForExportRegister.searchParam);
   }
   //禁用监听事件
	PartsAccountForExportRegister.grid.un("rowdblclick",PartsAccountForExportRegister.grid.toEditFn,PartsAccountForExportRegister.grid);
	 PartsAccountForExportRegister.grid.store.on('beforeload',function(){
	     var searchParam=PartsAccountForExportRegister.searchParam;
