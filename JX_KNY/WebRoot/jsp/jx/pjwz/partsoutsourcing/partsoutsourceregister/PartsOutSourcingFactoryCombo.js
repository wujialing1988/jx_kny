 ...
 * 车型选择列表
 * 控件使用方法：
 * 1.在页面jsp中引入js文件:<script language="javascript" src="<%=ctx%>/jsp/jx/pjwz/partsoutsourcing/PartsOutSourcingFactoryCombo.js"></script> 
 * 2.在表单页面js添加下例标签代码 
 * {id:自定义, xtype: 'partsOutSourcingFactory_combo',hiddenName: 自定义,fieldLabel:自定义,
 ...
 *   componentId 控件id
 */
Ext.namespace("jx.js.component.partsoutsourcing");
jx.js.component.partsoutsourcing.partsOutSourcingFactory_combo = function(){
	...
	this.isCx = 'yes';//设置是选择所有车型还是承修车型,默认查询承修车型
	this.dataurl = ctx + '/partsOutSourcingFactory!OutSourcingFactoryList.action';//数据源路径
	this.fields = ["id","factoryName"];//数据字段
	jx.js.component.partsoutsourcing.partsOutSourcingFactory_combo.superclass.constructor.apply(this, arguments);
	
}
Ext.extend(jx.js.component.partsoutsourcing.partsOutSourcingFactory_combo,Ext.form.ComboBox,{	
	 setStore : function(){		
			this.store = new Ext.data.JsonStore({
				root:"root", totalProperty:"totalProperty", 
				url: this.dataurl+'?queryParams='+Ext.util.JSON.encode(this.queryParams)+'&queryHql='+this.queryHql+'&isCx='+this.isCx,
				fields:this.fields
			});
			if(this.pageSize != 0){				
				this.store.load({
...
Ext.reg('PartsOutSourcingFactory_combo', jx.js.component.partsoutsourcing.partsOutSourcingFactory_combo);