Ext.onReady(function(){
	     ...
	     handler: function(){
		   ...
		   for(var i=0;i<data.length;i++){
		 	...
		 	Ext.Msg.confirm('提示', "是否确认撤销？", function(btn){
	    		...
	    		var sm = PartsCancelRegisterSearch.grid.getSelectionModel();
			    if (sm.getCount() < 1) {
			        MyExt.Msg.alert("尚未选择一条记录！");
			        return;
			    }
			    ...
			    Ext.Ajax.request({
			        url: ctx + "/partsCancelRegister!updatePartsCancelRegisterForCheck.action",
			        params: {ids: ids},
			        success: function(response, options){
			            var result = Ext.util.JSON.decode(response.responseText);
			            if (result.errMsg == null) {
				  ...
				  handler: function(){PartsCancelRegisterSearch.checkQuery();}
				},{
	...
	PartsCancelRegisterSearch.grid.un('rowdblclick', PartsCancelRegisterSearch.grid.toEditFn, PartsCancelRegisterSearch.grid);
	 //公用checkbox查询方法
	PartsCancelRegisterSearch.status = "10,20";
	//状态多选按钮
	PartsCancelRegisterSearch.checkQuery = function(){
		    PartsCancelRegisterSearch.status = "-1";
		if(Ext.getCmp("status_ed").checked){
			PartsCancelRegisterSearch.status = PartsCancelRegisterSearch.status + "," + status_ed;
		}
		if(Ext.getCmp("status_checked").checked){
			PartsCancelRegisterSearch.status = PartsCancelRegisterSearch.status + "," + status_checked;
		}
		PartsCancelRegisterSearch.grid.store.load();
	}
	//查询前添加过滤条件
	PartsCancelRegisterSearch.grid.store.on('beforeload',function(){
		...
		searchParam.status = PartsCancelRegisterSearch.status;
