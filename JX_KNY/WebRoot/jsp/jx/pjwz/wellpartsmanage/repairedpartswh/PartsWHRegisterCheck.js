Ext.onReady(function(){
    ...
    tbar: [{ text:'确认', iconCls:'checkIcon', handler:function(){
	    		var sm = PartsWHRegisterSearch.grid.getSelectionModel();
			    if (sm.getCount() < 1) {
			    ...
			    Ext.Ajax.request({
			        url: ctx + "/partsWHRegister!updatePartsWHRegisterForCheck.action",
			        params: {ids: ids},
			        success: function(response, options){
			            var result = Ext.util.JSON.decode(response.responseText);
			            if (result.errMsg == null) {
			                alertSuccess();
			                PartsWHRegisterSearch.grid.store.reload(); 
			            } else {
				  ...
				  handler: function(){PartsWHRegisterSearch.checkQueryS();}
				},{
	...
	fields: [{
			...
			header:'单据类型', dataIndex:'billType',
			renderer:function(v){
				if(v == BILLTYPE_SELF ) 
					return '自修';
	...
	PartsWHRegisterSearch.checkQuery = function(){
	 ...
	 //公用checkbox查询方法
	PartsWHRegisterSearch.status = "10,20";
	//状态多选按钮
	PartsWHRegisterSearch.checkQueryS = function(){
		    PartsWHRegisterSearch.status = "-1";
		if(Ext.getCmp("status_ed").checked){
			PartsWHRegisterSearch.status = PartsWHRegisterSearch.status + "," + status_ed;
		}
		if(Ext.getCmp("status_checked").checked){
			PartsWHRegisterSearch.status = PartsWHRegisterSearch.status + "," + status_checked;
		}
		PartsWHRegisterSearch.grid.store.load();
	}
	...
	PartsWHRegisterSearch.grid.store.on('beforeload',function(){
		...
		searchParam.status = PartsWHRegisterSearch.status;
