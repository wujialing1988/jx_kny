Ext.onReady(function(){
    ...
    tbar: [{ text:'确认', iconCls:'checkIcon', handler:function(){
	    		var sm = wellPartsRegisterSearch.grid.getSelectionModel();
			    if (sm.getCount() < 1) {
			    ...
			    Ext.Ajax.request({
			        url: ctx + "/wellPartsRegister!updateWellPartsRegisterForCheck.action",
			        params: {ids: ids},
			        success: function(response, options){
			            var result = Ext.util.JSON.decode(response.responseText);
			            if (result.errMsg == null) {
			                alertSuccess();
			                wellPartsRegisterSearch.grid.store.reload(); 
			            } else {
				  ...
				  handler: function(){wellPartsRegisterSearch.checkQueryS();}
				},{
	...
	//公用checkbox查询方法
	wellPartsRegisterSearch.status = "10,20";
	//状态多选按钮
	wellPartsRegisterSearch.checkQueryS = function(){
		    wellPartsRegisterSearch.status = "-1";
		if(Ext.getCmp("status_ed").checked){
			wellPartsRegisterSearch.status = wellPartsRegisterSearch.status + "," + status_ed;
		}
		if(Ext.getCmp("status_checked").checked){
			wellPartsRegisterSearch.status = wellPartsRegisterSearch.status + "," + status_checked;
		}
		wellPartsRegisterSearch.grid.store.load();
	}
	//查询前添加过滤条件
	wellPartsRegisterSearch.grid.store.on('beforeload',function(){
		...
		searchParam.status = wellPartsRegisterSearch.status;
