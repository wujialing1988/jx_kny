...
DeportSelect2_comboTree = function(){
	this.tree =  new Ext.tree.TreePanel({
	    loader : new Ext.tree.TreeLoader( {
	        dataUrl : ctx + '/omOrganizationSelect!deportTree2.action'
	    }),
	    root : new Ext.tree.AsyncTreeNode({id:'0',text: orgRoot}),
	    autoScroll : true,
	    rootVisible: true,
	    listeners: {
	        beforeload: function(node, e){
	            this.tree.getLoader().dataUrl = ctx + '/omOrganizationSelect!deportTree2.action?parentIDX=' + node.id + '&queryHql='+this.queryHql;
	        },
	        scope:this
	    }
	});
    ...
    DeportSelect2_comboTree.superclass.constructor.apply(this, arguments);
}

Ext.extend(DeportSelect2_comboTree,Ext.form.ComboBox, {
    
    expand : function(){
    	var ts = this;
        DeportSelect2_comboTree.superclass.expand.call(this);
        ...
        if(returnField != null && returnField.length > 0){        	
        	for(var i = 0;i < returnField.length;i++){
				...
				if(typeof(Ext.getCmp(returnField[i].widgetId)) != 'undefined' && Ext.getCmp(returnField[i].widgetId) != null 
					...
					Ext.getCmp(returnField[i].widgetId).reset();
...
Ext.reg('DeportSelect2_comboTree', DeportSelect2_comboTree);
