 ...
 * 2.在表单页面js添加下例标签代码 
 * { xtype:'GyjcFactory_SelectWin',editable:false,fieldLabel : 自定义,hiddenName:自定义,returnField:[{widgetId:"自定义",propertyName:"自定义"}}
 * 3.配置项说明：
 * a.id：控件id，为回显值必填
...
Factory_store = new Ext.data.JsonStore({
	url : ctx + "/gyjcFactory!factoryList.action",
	autoLoad : true,
	root : "root",
	remoteSort : true,
	totalProperty : "totalProperty",
	fields : [ "fID","fName","shortName","code","superFID","fcID"]
});
//分页工具
	...
	setValue : function(r) {				
		...
		{					
			if (this.hiddenField && typeof(this.hiddenField) != 'undefined') {
				this.hiddenField.value = typeof(r.get("fID")) == 'undefined' || r.get("fID")=='undefined'?'':r.get("fID");
			}
			Factory_Select.superclass.setValue.call(this, r.get("fName"));
			this.value = typeof(r.get("fID")) == 'undefined' || r.get("fID")=='undefined'?'':r.get("fID");			
		}				
		return this;				
	},
	//设置自定义配置项的回显值
...
Ext.reg('GyjcFactory_SelectWin', Factory_Select);