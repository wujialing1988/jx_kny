    ...
    tbar:[{
		...
		handler : function(){
			...
			if(querytype == '车号'){
				...
				searchParam.trainTypeShortName = typeName;
				Train.grid.getStore().load({
					params:{
						entityJson:Ext.util.JSON.encode(searchParam)
					}																
				...
				searchParam.holdOrgName = typeName;
				Train.grid.getStore().load({
					params:{
						entityJson:Ext.util.JSON.encode(searchParam)
					}																
...
Train.selectWin = new Ext.Window({
	title:"选择机车", maximizable:true,width:800, height:460, closeAction:"hide", modal:true, layout:"fit", items:Train.grid
});
//移除监听
