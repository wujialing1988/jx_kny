...
TrainEnforcePlan.grid = new Ext.yunda.Grid({
	...
	beforeSaveFn: function(data){
		// 默认状态为【审批完成】
		data.planStatus = status_audited;
		return true;
	},
	/**
...
TrainEnforcePlan.grid.saveWin  = new Ext.Window({
    title: "新增", maximizable: true, width: 900, height: 630, 
    layout: "border", plain: true, 
    closeAction: "hide", modal: true,
    items: [{   		
	    region: 'north',  
	    autoHeight: true,
	    style: "padding:5px",baseCls: "x-plain", 	    
	    items: TrainEnforcePlan.grid.saveForm,split: false
	   },{
	    region: "center",
		style: "padding:5px",
		items: [TrainEnforcePlanDetail.grid],
		layout: 'fit',
		title: '计划明细'
	 }]
});
...
TrainEnforcePlan.grid.store.on('beforeload' , function(){
		var searchParam = {} ;
		searchParam.planStartDate = TrainEnforcePlan.getCurrentYear("begin") ;
		searchParam.toPlanStartDate = TrainEnforcePlan.getCurrentYear("end") ;
