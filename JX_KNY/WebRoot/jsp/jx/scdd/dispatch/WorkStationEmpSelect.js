...
function del(empid){
	...
	for(var i = 0; i < store.getCount(); i++){
		var record = store.getAt(i);
		if(record.get("empid") == empid){
//			if (record.get("empstatus") == worker_status_handled) {
//				MyExt.Msg.alert("此作业人员已处理工单，不能删除");
//				return;
//			}
//			if (record.get("empstatus") == worker_status_waitingforget || record.get("empstatus") == worker_status_waitingforhandle 
//	    		|| record.get("empstatus") == worker_status_handling || Ext.isEmpty(record.get("empstatus"))) {
//	    			var noHandledCount = 0;
//				for (var j = 0; j < WorkStationEmp.grid.store.getCount(); j++) {
//					var newRecord = WorkStationEmp.grid.store.getAt(j);
//					if (newRecord.get("empstatus") == worker_status_waitingforget || newRecord.get("empstatus") == worker_status_waitingforhandle 
//	    				|| newRecord.get("empstatus") == worker_status_handling || Ext.isEmpty(newRecord.get("empstatus"))) {
//	    					noHandledCount++;
//	    				}
//				}
//				if (noHandledCount < 2) {
//					MyExt.Msg.alert("至少需选择一位未处理工单的作业人员");
//					return;
//				}
//				
//			}
...
Ext.onReady(function(){
	...
	WorkStationEmp.grid = new Ext.yunda.Grid({
	    loadURL: ctx + '/omEmployeeSelect!findListForWorkStationEmpSelect.action',                 //装载列表数据的请求URL    
