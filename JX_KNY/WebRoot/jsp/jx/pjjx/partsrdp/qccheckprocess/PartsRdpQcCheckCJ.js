...
Ext.onReady(function(){
	...
	Ext.namespace('PartsRdpQcCheckCJ');			// 定义命名空间
	
	/** ************** 定义全局变量开始 ************** */
	PartsRdpQcCheckCJ.labelWidth = 100;
	PartsRdpQcCheckCJ.fieldWidth = 140;
	PartsRdpQcCheckCJ.searchParams = {};
	PartsRdpQcCheckCJ.qcItems = [];
	PartsRdpQcCheckCJ.qcContent = "";
	/** ************** 定义全局变量结束 ************** */
	
	/** ************** 定义全局函数结束 ************** */
	PartsRdpQcCheckCJ.checkQueryFn = function() {
		PartsRdpQcCheckCJ.qcContent = "-1";
		for (var i = 0; i < PartsRdpQcCheckCJ.qcItems.length; i++) {
			var qcItem = PartsRdpQcCheckCJ.qcItems[i];
			if (Ext.getCmp(qcItem.id).checked) {
				PartsRdpQcCheckCJ.qcContent += "," + qcItem.inputValue;
			}
		}
		PartsRdpQcCheckCJ.grid.store.load();
	}
	// 初始化质量检查项的过滤条件
	PartsRdpQcCheckCJ.initQcItemsFn = function() {
		for (var i = 0; i < objList.length; i++) {
			var qcItem = objList[i];
			if (qcItem.checkWay != CHECK_WAY_CJ) {
				continue;
			}
			var editor = {};
	        ...
	        editor.width = 65;
	        editor.handler = PartsRdpQcCheckCJ.checkQueryFn;
	        PartsRdpQcCheckCJ.qcItems.push(editor);
		}
	}();
	/** ************** 定义全局函数开始 ************** */
	
	/** ************** 定义查询表单开始 ************** */
	PartsRdpQcCheckCJ.grid = new Ext.yunda.Grid({
		loadURL: ctx + '/partsRdp!findPartRdpQCItems.action',
//		page:false,
		saveForm:PartsRdpQcCheckProcess.baseForm,
		saveWin:PartsRdpQcCheckProcess.win,
		tbar: [PartsRdpQcCheckCJ.qcItems, '-', 'refresh'],
		fields: [{
			...
			renderer: function(value, metaData, record, rowIndex, colIndex, store){ 
				if (Ext.isEmpty(value)) {
	     		...
	     		var html = "";
	  			html = "<span><a href='#' onclick='PartsRdpQcCheckCJ.grid.toEditFn(\""+ PartsRdpQcCheckCJ.grid + "\",\""+ rowIndex +"\")'>"+value+"</a></span>";
	      		return html;
			}
		}, {
	...
	PartsRdpQcCheckCJ.grid.store.on('beforeload', function(){
		var searchParams = 	MyJson.deleteBlankProp(PartsRdpQcCheckCJ.searchParams);
		this.baseParams.entityJson = Ext.util.JSON.encode(searchParams);
		this.baseParams.qcContent = PartsRdpQcCheckCJ.qcContent;
		this.baseParams.checkWay = CHECK_WAY_CJ;
	});
	/** ************** 定义查询表单结束 ************** */
