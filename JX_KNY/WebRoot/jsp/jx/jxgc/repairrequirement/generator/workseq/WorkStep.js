/**
 * 工步（作业项-质量控制配置在作业任务上） 表示层，基于Extjs来构造显示页面，使用ajax json实现数据交互
 */
...
WorkStep.grid = new Ext.yunda.Grid({
    ...
    saveURL: ctx + '/workStep!saveWorkStep.action',             //保存数据的请求URL
    saveFormColNum:2,
    labelWidth: 120,
    fieldWidth: 240,
    tbar:[{
	...
	fields: [{
		...
		header:'技术要求或标准规定', dataIndex:'repairStandard', editor:{xtype:'textarea', maxLength:1000, width: 330,height:35, allowBlank:false},searcher: {disabled: true}		
	},{
...
WorkStep.stepForm = new Ext.FormPanel({
    ...
    //技术要求或标准规定  需求需要，修改字段长度为maxLength：1500
