...
Ext.onReady(function(){
	...
	DetectItem.loadMask = new Ext.LoadMask(Ext.getBody(), {msg:"正在处理，请等待......"});
	DetectItem.grid = new Ext.yunda.RowEditorGrid({
	    loadURL: ctx + '/detectResult!pageQuery.action',                 //装载列表数据的请求URL        
	    deleteURL: ctx + '/detectResult!logicDelete.action',
		...
		fields: [{
			...
			header:'工步主键', dataIndex:'workTaskIDX',hidden:true, editor:{ xtype:'hidden', maxLength:50 }
		},{
	    ...
	    beforeSaveFn: function(rowEditor, changes, record, rowIndex){
	        record.data.workTaskIDX = WorkStepEdit.idx ;
	    ...
	    beforeEditFn: function(rowEditor, rowIndex){
	     ...
	     saveFn: function(rowEditor, changes, record, rowIndex){
	        ...
	        if(DetectItem.loadMask)   DetectItem.loadMask.show();
	        var cfg = {
	            scope: this, url: this.saveURL, jsonData: record.data,
	            success: function(response, options){
	                if(DetectItem.loadMask)   DetectItem.loadMask.hide();
	                var result = Ext.util.JSON.decode(response.responseText);
	...
	DetectItem.grid.store.on('beforeload',function(){
		var searchParam = {};
		searchParam.workTaskIDX = WorkStepEdit.idx ; //检修工序卡主键
		var whereList = [] ;
