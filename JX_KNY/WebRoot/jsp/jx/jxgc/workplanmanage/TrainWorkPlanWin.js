...
//流水线排程
WorkPlanGantt.updateWorkCardForDispatchByRepairLine = function(){
	...
	if(Ext.isEmpty(task)) {
		...
		var parentIDX = task.nodeCaseIdx;
		
		//对子节点排程
		if(task.isLastLevel == CONST_INT_IS_LEAF_YES){
			if(task.status == NODE_STATUS_COMPLETE ){
					MyExt.Msg.alert("不能对【已完成】的节点进行流水线排程！");
					return;			
				}
				if(task.status == NODE_STATUS_STOP ){
					MyExt.Msg.alert("不能对【已终止】的节点进行流水线排程！");
					return;			
				}
			type = 2;
			dataParam = {};
			dataParam.rdpIDX = workPlanIDX;
			dataParam.nodeCaseIDX = task.nodeCaseIdx;
			dataParam.nodeStatus = task.status;		
			cascadingIDX = task.cascadingIDX;
			nodeIDX = task.nodeIDX;
			workStationIDX = task.workStationIDX;
			TecnodeUnionStation.grid.store.load();
			TecnodeUnionStation.selectWin.show();
		}
		//对非子节点排程
		else{
			var nodeCaseIDX = task.nodeCaseIdx;
			//根节点
			if(Ext.isEmpty(task.isLastLevel) && task.NodeType == PROCESSTYPE){
				if(task.status != WORKPLAN_STATUS_NEW){
					MyExt.Msg.alert("请选择【未启动】状态的节点！");
					return;			
				}
				type = 0;
				parentIDX = workPlanIDX;
				nodeCaseIDX = task.tecProcessCaseIDX;
			...
			Ext.Ajax.request({
	            ...
	            success: function(response, options){              	
	                ...
	                if (result.errMsg != null ) {
	                    ...
	                    RepairLineGroupSelect.tecProcessCaseIDX = task.tecProcessCaseIDX;
...
function showtip(msg){
	processTips = new Ext.LoadMask(TrainWorkPlanWin.win.getEl()||Ext.getBody(),{
...
ParentNodeTreeWin.returnFn = function(nodeIDX, changeNode){	
	var cfg = {
        url: ctx + '/jobProcessNode!changeNode.action',
...
Ext.namespace('TrainWorkPlanWin');                       //定义命名空间
TrainWorkPlanWin.fieldWidth = 120;
TrainWorkPlanWin.labelWidth = 80;
TrainWorkPlanWin.infoForm = new Ext.form.FormPanel({
	labelWidth:TrainWorkPlanWin.labelWidth, border: false,
...
TrainWorkPlanWin.ganttPanel = new Ext.Panel({
...
TrainWorkPlanWin.panel = new Ext.Panel({
    layout: "border", 
    defaults: {
    	border: false, layout: "fit"
    },
    items: [{
        ...
        items:[TrainWorkPlanWin.infoForm], frame: true, title: "检修基本信息"
    },{
        id:"gantt", region : 'center', items: TrainWorkPlanWin.ganttPanel
    }]
});
TrainWorkPlanWin.win = new Ext.Window({
    title:"车辆检修作业计划编辑", 
    closeAction:"hide", maximized : true, layout: 'fit',
    items:TrainWorkPlanWin.panel,
	listeners: {
		beforeshow: function() {
			// 加载甘特图数据
			WorkPlanGantt.loadFn();
...
TrainWorkPlanWin.showWin = function(data) {
