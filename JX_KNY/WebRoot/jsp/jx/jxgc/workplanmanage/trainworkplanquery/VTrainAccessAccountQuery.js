...
Ext.onReady(function(){
	...
	Ext.ns('VTrainAccessAccountQuery');						// 定义命名空间
	
	/** **************** 定义全局变量开始 **************** */ 
	VTrainAccessAccountQuery.labelWidth = 100;
	VTrainAccessAccountQuery.fieldWidth = 150;
	VTrainAccessAccountQuery.loadMask = new Ext.LoadMask(Ext.getBody(), {msg:"请稍候..."});
	
	VTrainAccessAccountQuery.editForm = new Ext.form.FormPanel({
		frame: true,
		padding: '10px',
	    labelWidth:VTrainAccessAccountQuery.labelWidth,
		layout:"column",
    	defaults: { 
			columnWidth:0.5,
    		xtype:"container", autoEl:"div", layout:"form",
    		defaults: {
	    		xtype:"textfield", width: VTrainAccessAccountQuery.fieldWidth
	    	}
    	},
		items:[{		    	
	    	items:[{
	    	...
	    	items:[{
				...
				listeners : {
					"beforequery" : function(){
						//选择车号前先选车型
						var trainTypeShortName =  VTrainAccessAccountQuery.editForm.getForm().findField("trainTypeShortName").getValue();
	...
	VTrainAccessAccountQuery.showStatisticsInfoFn = function(trainNo) {
		VTrainAccessAccountQuery.loadMask.show();
		Ext.Ajax.request({
			params: {trainNo: trainNo},
			url: ctx + '/trainAccessAccount!statistics.action',
			//请求成功后的回调函数
		    success: function(response, options){
				VTrainAccessAccountQuery.loadMask.hide();
	...
	/** **************** 定义信息面板开始 **************** */ 
	VTrainAccessAccountQuery.panel = new Ext.Panel({
		...
		collapsible: true, collapseMode: 'mini', collapsed: false,
		items: [{
			id: 'infoPanel',
			listeners: {
				render: function() {
		...
		tools:[{
		    ...
		    qtip: '查询在修机车分类信息',
		    handler: function() {
		    	if (!VTrainAccessAccountQuery.searchWin) {
		    		VTrainAccessAccountQuery.searchWin = new Ext.Window({
		    			title: '查询',
		    			closeAction: 'hide',
		    			...
		    			items: [{
		    				...
		    				listeners: {
		    					keyup: function(self, e) {
									...
									if (e.getKey() == e.ENTER){
		    							VTrainAccessAccountQuery.showStatisticsInfoFn(trainNo);
									}
					    		}
		    				}
		    			}],
		    	...
		    	VTrainAccessAccountQuery.searchWin.show();
		    }
		}, {
		    id:'refresh',
		    qtip: '刷新在修机车分类信息',
