...
Ext.onReady(function(){	
	...
	function showtip(msg){
		processTips = new Ext.LoadMask(TrainWorkPlanWinQuery.win.getEl()||Ext.getBody(),{
	...
	Ext.namespace('TrainWorkPlanWinQuery');                       //定义命名空间
	TrainWorkPlanWinQuery.fieldWidth = 120;
	TrainWorkPlanWinQuery.labelWidth = 80;
	
	TrainWorkPlanWinQuery.ganttPanel = new Ext.Panel({
	...
	TrainWorkPlanWinQuery.win = new Ext.Window({
	    title:"机车检修作业计划查看", 
	    closeAction:"hide", maximized : true, layout: 'fit',
	    items:TrainWorkPlanWinQuery.ganttPanel,
		listeners: {
			beforeshow: function() {
				// 加载甘特图数据
	...
	TrainWorkPlanWinQuery.showWin = function(data) {
	...
	//	form.findField("planEndTime").setValue(dataArray[4]);	
		...
		workPlanIDX = TrainWorkPlanGantt.workPlanIDX;
		 TrainWorkPlanWinQuery.win.show();         			
