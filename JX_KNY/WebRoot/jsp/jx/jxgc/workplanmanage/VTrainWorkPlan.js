	...
	VTrainWorkPlan.storeLoadFn = function(store, records, options ) {
		...
		for (var i = 0; i < count; i++) {
			...
			VTrainWorkPlan.plans.update({
				...
				content :  i + 1 + "、" + "计划：" + VTrainWorkPlan.formatPlanContentFn(plan, true)
			});
			// 添加分组信息【实际】
			VTrainWorkPlan.plans.update({
				id : groupId,
				title :  VTrainWorkPlan.formatPlanTitleFn(plan),
				content :  i + 1 + "、" + "<b>实际：</b>" + VTrainWorkPlan.formatPlanContentFn(plan, false)
			});
			...
			for (var j = 0; j < items.length; j++) {
				...
				VTrainWorkPlan.nodes.update({
					...
					className : className,
					group : groupId,
					content : VTrainWorkPlan.formatNodeContentFn(node, false, plan, className),
		...
		items: [{
			...
			tbar: [{
				...
				text: '在新窗口中打开', iconCls: 'previewIcon', handler: function() {
					window.open("VTrainWorkPlan.jsp");
				}
			}, '->', {
