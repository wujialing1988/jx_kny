...
//	TrainWorkPlanRecords.loadMask = new Ext.LoadMask(Ext.getBody(), { msg: "正在处理，请稍侯..." });
	...
	TrainWorkPlanRecords.setInfoForm = function(){
		...
		if (sm.getCount() > 0) {			
			...
			LeafNodeListGrid.workPlanIDX = records[0].data.idx;
		 	LeafNodeListGrid.processIDX = records[0].data.processIDX;
		 	LeafNodeListGrid.workPlanStatus = records[0].data.workPlanStatus;
			form.reset();
			...
			if(records[0].data.workPlanStatus == WORKPLAN_STATUS_NEW){
				form.findField("workPlanStatus").setValue("未启动");
			}else if(records[0].data.workPlanStatus == WORKPLAN_STATUS_HANDLING){
...
//					}
	...
	TrainWorkPlanRecords.trainTypeGrid.on("rowclick", function(grid, rowIndex, e){
	       	TrainWorkPlanFlowsheet.clearFn();
	       	TrainWorkPlanRecords.setInfoForm();
			TrainWorkPlanFlowsheet.store.reload();
	});
	//移除侦听器
...
//	        collapsible : true, minSize: 160, maxSize: 400, split: true, autoScroll: true,
	...
	TrainWorkPlanRecords.viewport = new Ext.Viewport({
		...
		items:[{
			layout: 'border',
			items: [
				TrainWorkPlanFlowsheet.TrainRecordPanel
		   ,{ 		
		     	 region: 'west',  title: '车辆检修计划',  bodyBorder: false, split: true, width: 240, layout: 'fit',   minSize: 160, maxSize: 400, 
