...
Ext.onReady(function(){
		...
		if (Ext.isEmpty(TrainWorkCardNew.tpl)) {
			TrainWorkCardNew.tpl = new Ext.XTemplate(
				'<table class="pjjx-show-info-table">',
					'<tr>',
		                '<td class="pjjx-show-info-table-label" width="12%">检修记录单编号：</td><td width="13%">{activityCode}</td>',
   ...
   // 加载记录卡详情
	...
	TrainWorkCardNew.win = new Ext.Window({
		...
		layout: 'border',
		items: [{
			region: 'north', height: 100, layout: 'fit', frame: true, title: '机车检修记录单信息',collapsible: true,
			items: {
				xtype: 'fieldset',
