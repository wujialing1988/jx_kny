...
Ext.onReady(function(){
	Ext.namespace('JobProcessDefNew');                       //定义命名空间
	
	/** **************** 定义全局变量开始 **************** */
	JobProcessDefNew.labelWidth = 100;
	JobProcessDefNew.fieldWidth = 140;
	JobProcessDefNew.serachParams = {};
	JobProcessDefNew.trainTypeIDX = "";
	JobProcessDefNew.isSaveAndAdd = false;
	
	JobProcessDefNew.trainShortName = "";			// 车型简称
	JobProcessDefNew.rcName = "";					// 修程名称
	
	/** **************** 定义全局变量结束 **************** */
	
	/** **************** 定义全局函数开始 **************** */
	// 复选框多选查询函数处理
	JobProcessDefNew.checkQueryFn = function() {
		JobProcessDefNew.status = [-1];
		if(Ext.getCmp("status_xz").checked){
			JobProcessDefNew.status.push(STATUS_XZ);
		} 
		if(Ext.getCmp("status_qy").checked){
			JobProcessDefNew.status.push(STATUS_QY);
		} 
		if(Ext.getCmp("status_zf").checked){
			JobProcessDefNew.status.push(STATUS_ZF);
		} 
		JobProcessDefNew.grid.store.load();
	}
	
	// 更新记录状态的函数处理
	JobProcessDefNew.updateStatusFn = function(status) {
		var sm = JobProcessDefNew.grid.getSelectionModel();
		if (sm.getCount() <= 0) {
	...
	// 配置流程节点
	JobProcessDefNew.configNodeFn = function() {
		var sm = JobProcessDefNew.grid.getSelectionModel();
		if (sm.getCount() <= 0) {
			MyExt.Msg.alert('尚未选择一条记录！');
			return;
	...
	// 自动设置【流程编码】字段值
	JobProcessDefNew.setProcessCodeFn = function() {
		Ext.Ajax.request({
			...
			success: function(response, options){
		        ...
		        if (result.errMsg == null) {
					JobProcessDefNew.saveForm.find('name', 'processCode')[0].setValue(result.rule);
				}
			},
	...
	/** **************** 定义编辑表单开始 **************** */
	JobProcessDefNew.saveForm = new Ext.form.FormPanel({
		labelWidth: JobProcessDefNew.labelWidth,
		layout:"column", padding:"10px", frame: true,
		defaults: {
			xtype:"container", autoEl:"div", layout:"form", columnWidth:0.5,
			defaults:{
				xtype: 'textfield', width: JobProcessDefNew.fieldWidth, maxLength: 50, allowBlank: false
			}
		},
		items:[{
			items:[{
				...
				listeners : {
		        	"select" : function(combo, record, index) {   
		                ...
		                rc_comb.cascadeStore();
	                 	...
	                 	// 选择【修程】后自动设置【流程名称】
	                 	JobProcessDefNew.trainShortName = record.get('typeCode');			// 车型简称
						JobProcessDefNew.rcName = "";
		        	}
		    	}
			}, {
				xtype:"textfield",
				...
				xtype:"numberfield",
				fieldLabel:"额定工期(小时)",
				labelWidth: JobProcessDefNew.labelWidth + 30,
				name:"ratedWorkDay",
				vtype:"positiveInt",
				value:1,
				maxLength: 6
			...
			items:[{
    			...
    			listeners: {
                	...
                	select : function(combo, record, index) {
                		JobProcessDefNew.rcName = record.get('xcName');
                		var processNameField = this.findParentByType('form').find('name', 'processName')[0];
                		if (!Ext.isEmpty(JobProcessDefNew.rcName) && !Ext.isEmpty(JobProcessDefNew.trainShortName)) {
                			processNameField.setValue(JobProcessDefNew.trainShortName + JobProcessDefNew.rcName + '作业流程')
                		}
                	}
                }
	...
	JobProcessDefNew.grid = new Ext.yunda.Grid({
	    ...
	    saveForm: JobProcessDefNew.saveForm,
	    tbar: ['add', 'delete', '-', {
	    	text: '启用', iconCls: '', handler: function() {
	    		JobProcessDefNew.updateStatusFn(STATUS_QY);
	    	}
	    }, {
	    	text: '作废', iconCls: '', handler: function() {
	    		JobProcessDefNew.updateStatusFn(STATUS_ZF);
	    	}
	    }, '-', {
	    	text: '设置流程节点', iconCls: 'configIcon', handler: JobProcessDefNew.configNodeFn
	    }, {
...
//	    		JobProcessGantt.win.setTitle(sm.getSelections()[0].get('processName'));
	    	...
	    	xtype:"checkbox", id:"status_xz", boxLabel:"新增&nbsp;&nbsp;&nbsp;&nbsp;", checked:true , handler: JobProcessDefNew.checkQueryFn
	    }, {
	    	xtype:"checkbox", id:"status_qy", boxLabel:"启用&nbsp;&nbsp;&nbsp;&nbsp;", checked:true , handler: JobProcessDefNew.checkQueryFn
	    }, {
	    	xtype:"checkbox", id:"status_zf", boxLabel:"作废&nbsp;&nbsp;&nbsp;&nbsp;", checked:false , handler: JobProcessDefNew.checkQueryFn
	    }, 'refresh', '->', '车型：', {
			...
			listeners : {   
	        	"select" : function() {
	        		JobProcessDefNew.trainTypeIDX = this.getValue();
	        		// 重新加载表格数据
	        		JobProcessDefNew.grid.store.load();
	        	}   
	    	}
		...
		fields: [{
			...
			header:'流程编码', dataIndex:'processCode', width: 140, editor:{  maxLength:50 }, renderer: function(value, metaData, record){
				var html = "";
	  			html = "<span><a href='#' onclick='JobProcessDefNew.configNodeFn()'>"+value+"</a></span>";
	      		return html;
			}
		},{
			...
			header:'车型', dataIndex:'trainTypeShortName', editor:{  maxLength:50 }
		},{
			header:'车型名称', dataIndex:'trainTypeName', hidden:true, editor:{  maxLength:50 }
		},{
...
//	            width:600, height:340, 
	            ...
	            items: {
		            ...
		            buttons: [{
		                ...
		                text: "保存并新增", iconCls: "addIcon", scope: this, handler: function(){
		                	JobProcessDefNew.isSaveAndAdd = true;
		                	this.saveFn();
		                }
		            }, {
	    ...
	    afterShowSaveWin: function(){
	    	...
	    	// 自动设置作业流程编码
	    	JobProcessDefNew.setProcessCodeFn();
	    	var cfg = {
		        scope: this, url: ctx + '/workCalendarInfo!getWorkCalendarInfo.action',
		        success: function(response, options){
		            var result = Ext.util.JSON.decode(response.responseText);
		            if (result.defInfo != null) {
		                JobProcessDefNew.saveForm.getForm().findField("workCalendarIDX").setDisplayValue(result.defInfo.idx, result.defInfo.calendarName);
		            }
		        }
		    };
		    Ext.Ajax.request(Ext.apply($yd.cfgAjaxRequest(), cfg));
	    ...
	    afterSaveSuccessFn: function(result, response, options){
	        ...
	        alertSuccess();
	        // 【保存并新增】
	        if (JobProcessDefNew.isSaveAndAdd) {
	        	// 重置表单
	...
	JobProcessDefNew.grid.store.setDefaultSort('status', 'ASC'),
	JobProcessDefNew.grid.store.on('beforeload', function(){
		var searchParams = JobProcessDefNew.serachParams;
		searchParams.trainTypeIDX = JobProcessDefNew.trainTypeIDX;
		// 初始化只查询【启用】的检修作业流程
		if (undefined == JobProcessDefNew.status) {
			searchParams.status = [STATUS_XZ, STATUS_QY];
		} else {
			searchParams.status = JobProcessDefNew.status;
		}		
		// 删除空属性
	...
	//页面适应布局
	var viewport = new Ext.Viewport({ layout:'fit', items:JobProcessDefNew.grid });
});