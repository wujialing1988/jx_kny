...
Ext.onReady(function() {
	...
	/** ************** 定义全局函数开始 ************** */
	// 查询结束后，计算列表总金额
	MatTrainExpendAccountQuery.calculateTotalPriceFn = function() {
		var store = MatTrainExpendAccountQuery.grid.store;
		var totalPrice = 0;
	...
	MatTrainExpendAccountQuery.grid = new Ext.yunda.Grid({
		...
		fields: [{
			...
			header: '总价（元）', dataIndex: '', width: 30, 
			...
			header: '状态', dataIndex: 'status', width: 30, hidden:false, renderer: function(v) {
	...
	// 数据容器数据改变后，重新计算【总金额（元）】
	MatTrainExpendAccountQuery.grid.store.on('datachanged', MatTrainExpendAccountQuery.calculateTotalPriceFn);
	/** ************** 定义主体表格结束 ************** */
