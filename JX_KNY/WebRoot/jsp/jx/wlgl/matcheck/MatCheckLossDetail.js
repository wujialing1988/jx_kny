...
Ext.onReady(function(){
	...
	Ext.namespace('MatCheckLossDetail');                       //定义命名空间
	...
	/** ************* 定义全局函数开始 ************* */
	// 【添加】按钮触发的函数处理
	MatCheckLossDetail.addFn = function() {
			...
			success : function(response, options) {
				...
				if (result.errMsg == null) {
					...
					for (var i = 0; i < list.length; i++) {
						var entity = list[i];
						var count = MatCheckLossDetail.grid.store.getCount();
						if (count != 0) {
							for (var i = 0; i < count; i++) {
								var record = MatCheckLossDetail.grid.store.getAt(i);
					...
					for (var j = 0; j < list.length; j++) {
						...
						record_v.set("qty","1");
						MatCheckLossDetail.grid.store.insert(0, record_v); 
					    MatCheckLossDetail.grid.getView().refresh(); 
					    MatCheckLossDetail.grid.getSelectionModel().selectRow(0)
					}
				} else {
	...
	// 【删除】按钮触发的函数处理
	MatCheckLossDetail.deleteFn = function() {
        var data = MatCheckLossDetail.grid.selModel.getSelections();
        if(data.length == 0 ){
        	MyExt.Msg.alert("尚未选择一条记录！");
            return;
        }
        var storeAt = MatCheckLossDetail.grid.store.indexOf(data[0]);
        var records = MatCheckLossDetail.store.getModifiedRecords();
        var count = records.length; 
        var j = storeAt + 1;
        if(count-1 == storeAt){
        	j = storeAt-1;
        }
	    MatCheckLossDetail.grid.getSelectionModel().selectRow(j);
	    for (var i = 0; i < data.length; i++){
	        MatCheckLossDetail.grid.store.remove(data[i]);
	    }
	    MatCheckLossDetail.grid.getView().refresh();
	}
	/** ************* 定义全局函数结束 ************* */
	
	MatCheckLossDetail.batchWin = new Ext.Window({
		title:"物料选择",
		...
		buttons: [{
			text:'确认', iconCls:'yesIcon', handler: function() {
				...
				// 添加成功后，隐藏【物料选择】 窗口
				MatCheckLossDetail.batchWin.hide();
			}
		}, {
	...
	// 数据容器
	MatCheckLossDetail.store = new Ext.data.JsonStore({
		id:'idx', totalProperty:'totalProperty', autoLoad:false, pruneModifiedRecords: true,
		root: 'root',
		url:ctx+'/matCheckLossDetail!pageList.action',
		fields:['idx','matCode','matCheckLossIDX','matDesc','unit','qty']
	});
	// 操作掩码
    MatCheckLossDetail.loadMask = new Ext.LoadMask(Ext.getBody(), {msg:"正在处理，请等待......"});
	// 行选择模式
    MatCheckLossDetail.sm = new Ext.grid.CheckboxSelectionModel({singleSelect:false});
    // 单据明细表格
    MatCheckLossDetail.grid = new Ext.grid.EditorGridPanel({
		border: false, enableColumnMove: true, stripeRows: true, selModel: MatCheckLossDetail.sm, loadMask: true,
		clicksToEdit: 1,
		viewConfig: {forceFit: true},
        store: MatCheckLossDetail.store,
		colModel:new Ext.grid.ColumnModel([
	        new Ext.grid.RowNumberer(),
			{
	        	...
	        	sortable:false, dataIndex:'matCheckLossIDX', hidden:true, header:'盘盈单idx主键'
	        },{	
...
//							if (e.getKey() == e.ENTER){
//								MatCheckLossDetail.addFn();
//							}
//						}
					}
				}, '&nbsp;&nbsp;', {
					text:'添加',iconCls:'addIcon',handler:MatCheckLossDetail.addFn
				}, '&nbsp;&nbsp;', {
					text:'批量添加',iconCls:'chart_attributeConfigIcon',handler:function(){
						...
						// 打开【物料选择】 窗口
	              		MatCheckLossDetail.batchWin.show();
						// 重新加载 【物料选择】 窗口表格数据
						MatTypeList.grid.store.load();
					}
				}, '&nbsp;&nbsp;', {
				text:'删除', iconCls:'deleteIcon', handler:MatCheckLossDetail.deleteFn
				}, '&nbsp;&nbsp;', {
	...
	MatCheckLossDetail.grid.store.on("beforeload", function(){
		var searchParams = {matCheckLossIDX: MatCheckLossDetail.matCheckLossIDX};
