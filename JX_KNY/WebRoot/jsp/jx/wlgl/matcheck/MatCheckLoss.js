...
Ext.onReady(function(){
	Ext.namespace('MatCheckLoss');                       //定义命名空间
	
	/** ************ 定义全局函数开始 ************ */
	// 【新增】按钮触发的页面初始化操作（初始化页面）
	MatCheckLoss.resetFn = function() {
		Ext.getCmp('basePanel').setTitle("单据信息<font color='green'>(暂存)</font>");
        // 清空【单据信息】列表
		if (MatCheckLossDetail.grid.store.getCount() > 0) {
	        MatCheckLossDetail.grid.store.removeAll();
		}
        // 重新设置matInWhIdx
        MatCheckLossDetail.matCheckLossIDX = "###";
		MatCheckLoss.isTemporary = true;
		Ext.getCmp('whName_k').enable();
		Ext.getCmp('checkEmpId_k').enable();
		Ext.getCmp('lossDate_k').enable();
//							Ext.getCmp('billSummary_k').enable();
		MatCheckLossDetail.grid.getTopToolbar().enable();
		MatCheckLoss.baseForm.getForm().reset();
		...
		MatCheckLoss.setMatInWHBillNo();

	}
	/** ************ 定义全局函数结束 ************ */
	
	/** ************ 定义全局变量开始 ************ */
	MatCheckLoss.loadMask = new Ext.LoadMask(Ext.getBody(), {msg:"正在处理，请等待......"});
	
	MatCheckLoss.fieldWidth = 100;
	MatCheckLoss.labelWidth = 70;
	MatCheckLoss.loadMask = new Ext.LoadMask(Ext.getBody(), {msg:"正在处理，请等待......"});
	MatCheckLoss.isTemporary = true;					// 当前操作的记录是否为“未登账”记录的标识
	// 查询表单
	MatCheckLoss.searchForm = new Ext.form.FormPanel({
		labelWidth:MatCheckLoss.labelWidth,
		labelAlign:"left",
		...
		items:[
			{
				...
				items:[
					{
					xtype: 'compositefield', fieldLabel: '盘亏日期', combineErrors: false,
					items: [{
			...
			},
			{
				...
				items:[{
					...
					xtype:'button', text:'查询', iconCls:'searchIcon', width: 80, handler: function() {
							var form = MatCheckLoss.searchForm.getForm();
							if (!form.isValid()) {
								return;	
							}
							...
							MatCheckLoss.grid.store.load();
						}
					}]
				}, {
					columnWidth:0.25, layout: 'form', baseCls:"x-plain", border: false,
					items:[{
					xtype:'button', text:'新增', iconCls:'addIcon', width: 80, handler: MatCheckLoss.resetFn
					}]
				}, {
					columnWidth:0.25, layout: 'form', baseCls:"x-plain", border: false,
					items:[{
					xtype:'button', text:'删除', iconCls:'deleteIcon', width: 80, handler: function() {
							var sm = MatCheckLoss.grid.getSelectionModel();
							if (sm.getCount() <= 0) {
									MyExt.Msg.alert("尚未选择任何记录！");
									return;
							}
							if (!MatCheckLoss.isTemporary) {
								...
								Ext.Msg.confirm("提示  ", "该操作将不能恢复，是否继续？  ", function(btn){
							        if(btn == 'yes') {
								        Ext.Ajax.request(Ext.apply($yd.cfgAjaxRequest(), {
								        	scope: MatCheckLoss.grid,
								        	url: MatCheckLoss.grid.deleteURL,
											params: {ids: $yd.getSelectedIdx(MatCheckLoss.grid, MatCheckLoss.grid.storeId)},
											success: function(response, options) {
												// 重新加载主表格
												MatCheckLoss.grid.store.reload();
												// 初始化页面
												MatCheckLoss.resetFn();
											}
								        }));
							        }
							    });    
					...
					xtype:'button', text:'回滚', iconCls:'resetIcon', width: 80, handler: function() {
							...
							if (data.status == STATUS_ZC) {
								...
								Ext.Msg.confirm("提示  ", "是否确认回滚？  ", function(btn){
							        if(btn == 'yes') {
						        		MatCheckLoss.loadMask.show();
						        		Ext.Ajax.request(Ext.apply($yd.cfgAjaxRequest(), {
								        	scope: MatCheckLoss.grid,
								        	url: ctx + '/matCheckLoss!updateRollBack.action',
											params: {ids: $yd.getSelectedIdx(MatCheckLoss.grid, MatCheckLoss.grid.storeId)},
											success: function(response, options){
								              	MatCheckLoss.loadMask.hide();
	...
	// 单据列表
	MatCheckLoss.grid = new Ext.yunda.Grid({
		...
		listeners: {
			'rowclick': function(grid, rowIndex, e) {
				...
				MatCheckLossDetail.matCheckLossIDX = record.data.idx;
				MatCheckLossDetail.grid.store.load();
				// 重新加载【单据信息】表单
				MatCheckLoss.baseForm.getForm().loadRecord(record);
...
//					Ext.getCmp('billSummary_k').enable();
					...
					MatCheckLoss.isTemporary = false;
					Ext.getCmp('whName_k').disable();
					Ext.getCmp('checkEmpId_k').disable();
					Ext.getCmp('lossDate_k').disable();
//					Ext.getCmp('billSummary_k').disable();
					MatCheckLossDetail.grid.getTopToolbar().disable();
				}
			}
		}
	});
	MatCheckLoss.grid.un('rowdblclick', MatCheckLoss.grid.toEditFn, MatCheckLoss.grid);
	MatCheckLoss.grid.store.on('beforeload', function() {
		var searchParams = MatCheckLoss.searchForm.getForm().getValues();
		searchParams = MyJson.deleteBlankProp(searchParams);
		this.baseParams.entityJson = Ext.util.JSON.encode(searchParams);
	})
	
	// 【单据信息】表单
	MatCheckLoss.baseForm = new Ext.form.FormPanel({
		labelWidth:MatCheckLoss.labelWidth,
		layout:"column",
		padding:"10px",
		items:[
				...
				items:[
					{
						...
						xtype:"Base_combo",allowBlank: false,width:MatCheckLoss.fieldWidth,
	...
	/** ************ 定义全局函数开始 ************ */
	// 自动设置【单据编号】字段值
	MatCheckLoss.setMatInWHBillNo = function() {
		Ext.Ajax.request({
	...
	// 获取表单数据前激活被禁用的字段
	MatCheckLoss.beforeGetBaseDate = function() {
	...
	// 获取表单数据后禁用被激活的字段
	MatCheckLoss.afterGetBaseDate = function() {
	...
	MatCheckLoss.afterSaveTemporayFn = function() {
        // 重新加载【单据列表】列表
        MatCheckLoss.grid.store.reload();
        // 清空【单据信息】列表
        MatCheckLossDetail.grid.store.removeAll();
	...
	// 【暂存】按钮触发的函数操作
	MatCheckLoss.SaveTemporaryFn = function(isTemporary) {
		// 物料盘点单基本信息
		var form = MatCheckLoss.baseForm.getForm();
		// 验证表单数据的合法性
		if (!form.isValid()) {
			return;
		}
		MatCheckLoss.beforeGetBaseDate();
		var matCheckLoss =  form.getValues();
		if (MatCheckLossDetail.matCheckLossIDX != "###" && !Ext.isEmpty(MatCheckLossDetail.matCheckLossIDX)) {
			matCheckLoss.idx = MatCheckLossDetail.matCheckLossIDX;
		}
		// 判断是【暂存】还是【登账并新增】操作
		if (isTemporary) {
			// 设置单据状态为【暂存（temporary）】
			matCheckLoss.status = STATUS_ZC;
		} else {
			// 设置单据状态为【暂存（entryAccount）】
			matCheckLoss.status = STATUS_DZ;
		}
		MatCheckLoss.afterGetBaseDate();
		// 获取添加的明细数据
		var store = MatCheckLossDetail.store;
		// 验证添加的明细数据是否为空
		if(store.getCount() == 0){
		...
		MatCheckLoss.loadMask.show();
        Ext.Ajax.request({
            url: ctx + '/matCheckLoss!saveLossAndDetail.action',
            jsonData: datas,
            params : {matCheckLoss : Ext.util.JSON.encode(MyJson.deleteBlankProp(matCheckLoss))},
            success: function(response, options){
              	MatCheckLoss.loadMask.hide();
                var result = Ext.util.JSON.decode(response.responseText);
                if (result.errMsg == null) {
                    alertSuccess();
                    MatCheckLoss.afterSaveTemporayFn();
                } else {
                    alertFail(result.errMsg);
                }
            },
		...
		items:[
			{
				...
				items:[
					{
						...
						height:42,
						layout:"fit",
						items: MatCheckLoss.searchForm
					},
					{
						xtype:"panel", border: false,
						region:"center",
						layout:"fit",
						items: MatCheckLoss.grid
					}
				]
			}, {
				region:"center",
				...
				items:[
					...
					},
					{
						...
						layout:"fit",
						buttonAlign:"center",
						items:MatCheckLossDetail.grid,
						buttons:[{
							text:'暂存',handler: function() {
								if (MatCheckLoss.isTemporary) {
									MatCheckLoss.SaveTemporaryFn(true);
								} else {
							...
							text:'登账并新增',handler: function() {
								if (MatCheckLoss.isTemporary) {
									Ext.Msg.confirm("提示  ", "是否确认登账？  ", function(btn){
								        if(btn == 'yes') {
											MatCheckLoss.SaveTemporaryFn(false);
								        }
							        });
								} else {
									MyExt.Msg.alert("已登账的记录单不能再次操作");
