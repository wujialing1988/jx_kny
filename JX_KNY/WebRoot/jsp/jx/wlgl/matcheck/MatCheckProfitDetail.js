/**
 * 物料盘盈明细 表示层，基于Extjs来构造显示页面，使用ajax json实现数据交互
 */
Ext.onReady(function(){
	
	Ext.namespace('MatCheckProfitDetail');                       //定义命名空间
	...
	/** ************* 定义全局函数开始 ************* */
	// 【添加】按钮触发的函数处理
	MatCheckProfitDetail.addFn = function() {
			...
			success : function(response, options) {
				...
				if (result.errMsg == null) {
					...
					for (var i = 0; i < list.length; i++) {
						var entity = list[i];
						var count = MatCheckProfitDetail.grid.store.getCount();
						if (count != 0) {
							for (var i = 0; i < count; i++) {
								var record = MatCheckProfitDetail.grid.store.getAt(i);
					...
					for (var j = 0; j < list.length; j++) {
						...
						record_v.set("qty","1");
						MatCheckProfitDetail.grid.store.insert(0, record_v); 
					    MatCheckProfitDetail.grid.getView().refresh(); 
					    MatCheckProfitDetail.grid.getSelectionModel().selectRow(0)
					}
				} else {
	...
	MatCheckProfitDetail.deleteFn = function() {
        var data = MatCheckProfitDetail.grid.selModel.getSelections();
        if(data.length == 0 ){
        	MyExt.Msg.alert("尚未选择一条记录！");
            return;
        }
        var storeAt = MatCheckProfitDetail.grid.store.indexOf(data[0]);
        var records = MatCheckProfitDetail.store.getModifiedRecords();
        var count = records.length; 
        var j = storeAt + 1;
        if(count-1 == storeAt){
        	j = storeAt-1;
        }
	    MatCheckProfitDetail.grid.getSelectionModel().selectRow(j);
	    for (var i = 0; i < data.length; i++){
	        MatCheckProfitDetail.grid.store.remove(data[i]);
	    }
	    MatCheckProfitDetail.grid.getView().refresh();
	}
	// 【删除】按钮触发的函数处理
	...
	MatCheckProfitDetail.batchWin = new Ext.Window({
		title:"物料选择",
		...
		buttons: [{
			text:'确认', iconCls:'yesIcon', handler: function() {
				...
				// 添加成功后，隐藏【物料选择】 窗口
				MatCheckProfitDetail.batchWin.hide();
			}
		}, {
	...
	// 数据容器
	MatCheckProfitDetail.store = new Ext.data.JsonStore({
		id:'idx', totalProperty:'totalProperty', autoLoad:false, pruneModifiedRecords: true,
		root: 'root',
		url:ctx+'/matCheckProfitDetail!pageList.action',
		fields:['idx','matCode','matCheckProfitIDX','matDesc','unit','qty']
	});
	// 操作掩码
    MatCheckProfitDetail.loadMask = new Ext.LoadMask(Ext.getBody(), {msg:"正在处理，请等待......"});
	// 行选择模式
    MatCheckProfitDetail.sm = new Ext.grid.CheckboxSelectionModel({singleSelect:false});
    // 单据明细表格
    MatCheckProfitDetail.grid = new Ext.grid.EditorGridPanel({
		border: false, enableColumnMove: true, stripeRows: true, selModel: MatCheckProfitDetail.sm, loadMask: true,
		clicksToEdit: 1,
		viewConfig: {forceFit: true},
        store: MatCheckProfitDetail.store,
		colModel:new Ext.grid.ColumnModel([
	        new Ext.grid.RowNumberer(),
			{
	        	...
	        	sortable:false, dataIndex:'matCheckProfitIDX', hidden:true, header:'盘盈单idx主键'
	        },{	
...
//							if (e.getKey() == e.ENTER){
//								MatCheckProfitDetail.addFn();
//							}
//						}
					}
				}, '&nbsp;&nbsp;', {
					text:'添加',iconCls:'addIcon',handler:MatCheckProfitDetail.addFn
				}, '&nbsp;&nbsp;', {
					text:'批量添加',iconCls:'chart_attributeConfigIcon',handler:function(){
						// 打开【物料选择】 窗口
	              		MatCheckProfitDetail.batchWin.show();
						// 重新加载 【物料选择】 窗口表格数据
						MatTypeList.grid.store.load();
					}
				}, '&nbsp;&nbsp;', {
				text:'删除', iconCls:'deleteIcon', handler:MatCheckProfitDetail.deleteFn
				}, '&nbsp;&nbsp;', {
	...
	MatCheckProfitDetail.grid.store.on("beforeload", function(){
		var searchParams = {matCheckProfitIDX: MatCheckProfitDetail.matCheckProfitIDX};
