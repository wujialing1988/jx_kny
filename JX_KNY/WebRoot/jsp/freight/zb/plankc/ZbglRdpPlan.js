...
Ext.onReady(function(){
	...
	TrainDemandSelectWin.submit = function(){
		...
		ZbglRdpPlan.ZbglRdpPlanGrid.addButtonFn();
		var form = ZbglRdpPlan.ZbglRdpPlanGrid.saveForm.getForm();
		form.findField("trainDemandIdx").setValue(records[0].data.idx);
		form.findField("railwayTime").setValue(records[0].data.strips + "/" + records[0].data.backStrips);
		form.findField("rdpNum").setValue(records[0].data.trainCount);
		ZbglRdpPlan.ZbglRdpPlanGrid.setReadOnlyColumns(['rdpNum'],true);
	}
	...
	ZbglRdpPlan.setInfoForm = function(sm){
		...
		if (sm.getCount() > 0) {	
			...
			ZbglRdpPlanRecordCL.rdpPlanIdx = records[0].data.idx;
