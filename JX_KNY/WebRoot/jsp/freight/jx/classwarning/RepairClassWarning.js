...
RepairClassWarning.grid = new Ext.yunda.Grid({
    loadURL: ctx + '/repairClassWarning!findKCRepairClassWarningList.action',                 //装载列表数据的请求URL
    singleSelect: true, 
    saveFormColNum:1,
...
//    		}
    ...
    tbar:[
		...
		},'-','<div><span style="color:red;">*红色</span>表示该车辆走行公里超限，未进行检修;<span style="color:#FF7F00;">*橙色</span>表示该车辆走行公里到达警戒线，需进行检修！</div>'
    ],    
	fields: [
     	{
		...
		header:'累计走行(km)', dataIndex:'totalrm',width: 120,editor: {}
	}, {
		header:'最大走行(km)', dataIndex:'maxRunningKm',width: 120,editor: {}
	}, {
		header:'超限/临限公里(km)', dataIndex:'maxRunningKm',width: 120,editor: {},renderer: function(value, metaData, record, rowIndex, colIndex, store) {
		...
		header:'预警提示', dataIndex:'maxRunningKm',width: 200,editor: {},renderer: function(value, metaData, record, rowIndex, colIndex, store) {
	    		...
	    		var maxRunningKm = record.get('maxRunningKm'); // 最大
	    		if(parseFloat(maxRunningKm) > parseFloat(totalrm)){
					msg = "<span style='color:#FF7F00;'>车辆走行已达到警戒标准，请提前安排检修！</span>" ;
	    		}else{
					msg = "<span style='color:red;'>车辆走行已超过修程限度，请尽快安排检修！</span>" ;
	    		}
