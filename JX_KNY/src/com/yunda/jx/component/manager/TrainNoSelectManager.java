 ...
 * @version 1.0
 */
@Service(value = "trainNoSelectManager")
public class TrainNoSelectManager  extends JXBaseManager<JczlTrain, JczlTrain> implements IbaseCombo {
    ...
    public Map<String, Object> page(Map queryParams, int start, int limit) {
        ...
        if (StringUtil.isNullOrBlank(queryHql)) {
            ...
            if ("true".equals(isRemoveRun)) {// isRemoveRun为true代表需排除作业计划已有机车
                selectHql.append(" and trainNo not in (select trainNo from TrainWorkPlan where recordStatus=0 and workPlanStatus in ('")
                         .append(TrainWorkPlan.STATUS_HANDLING)
                         .append("','")
                         .append(TrainWorkPlan.STATUS_NEW)
                         .append("') and trainNo is not null");
