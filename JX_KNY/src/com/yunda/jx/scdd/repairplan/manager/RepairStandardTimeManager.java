 ...
 */
@Service
public class RepairStandardTimeManager extends JXBaseManager<RepairStandardTime, RepairStandardTime> {

	@Override
	public String[] validateUpdate(RepairStandardTime t) {
	 ...
	 * <li>修改日期：
	 */
	private boolean isDuplicateData(RepairStandardTime t){
		String hql = "select count(*) from RepairStandardTime where trainTypeIdx = ? and repairClass = ? and repairClassCompare = ?";
		List<String> params = new ArrayList<String>(4);
		params.add(t.getTrainTypeIdx());
		params.add(t.getRepairClass());
		params.add(t.getRepairClassCompare());
