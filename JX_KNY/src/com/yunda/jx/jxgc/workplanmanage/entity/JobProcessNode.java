...
/**
 * <li>标题: 机车检修管理信息系统
 * <li>说明：JobProcessNode实体类, 数据表：机车检修作业计划-流程节点
 * <li>创建人：程锐
 * <li>创建日期：2015-04-15
 * <li>修改人:
 * <li>修改日期：
 ...
 */
@Entity
@Table(name = "JXGC_Job_Process_Node")
@JsonAutoDetect(fieldVisibility = Visibility.ANY, getterVisibility = Visibility.NONE)
public class JobProcessNode implements java.io.Serializable {
    ...
    /*记录当前节点的前置节点*/
    @Transient
    private String lastChildNodeNames;
    ...
    /**
     * <li>说明：记录当前节点的前置节点
     * <li>返回值： the lastChildNodeNames
     */
    public String getLastChildNodeNames() {
        return lastChildNodeNames;
    }

    
    /**
     * <li>说明：记录当前节点的前置节点
     * <li>参数： lastChildNodeNames
     */
    public void setLastChildNodeNames(String lastChildNodeNames) {
        this.lastChildNodeNames = lastChildNodeNames;
    }
     ...
     * <li>修改日期：
     * <li>修改内容：
     * @param o 机车检修作业计划-流程节点
     * @return 是否可以合并 true:是 false:否
     */
    public boolean canCombined(JobProcessNode o) {
        // 必须是同一个工作计划才可以合并
        if (!this.workPlanIDX.equals(o.workPlanIDX)) {
            return false;
        }
    ...
    public long getDelayTime(Date currentTime) {
         ...
         */
        if (STATUS_COMPLETE.equals(status) && null != realWorkMinutes) {
            delayTime = (realWorkMinutes.longValue() - ratedWorkMinutes.longValue()) * 60 * 1000;
        } else if (STATUS_GOING.equals(status)) {
            if (null == this.realBeginTime) {
                throw new BusinessException("流程节点：" + nodeName +"(" + idx +")实际开工时间为空！");
            }
            delayTime = currentTime.getTime() - (this.realBeginTime.getTime() + ratedWorkMinutes.longValue() * 60 * 1000);
        }
