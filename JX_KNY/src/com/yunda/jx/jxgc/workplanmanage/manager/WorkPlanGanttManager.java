 ...
 * @version 1.0
 */
@Service(value = "workPlanGanttManager")
public class WorkPlanGanttManager extends JXBaseManager<Object, Object> {
     ...
     * @throws Exception
     */
    public Result planOrderGantt(TrainWorkPlan workPlan, String displayMode, String nodeIdx) throws Exception {
    ...
    public Task[] getTasks(TrainWorkPlan workPlan) throws Exception {
        int i = 1;
        ...
        // 流程节点任务
        List<JobProcessNode> list = getNodeListByWorkPlanForGantt(workPlan.getIdx(), null);// parentIdx传null查询第一层节点
        Integer percentComplete = calPercentCompleteByList(list);// 完成百分比
        List<JobProcessNode> nodeList = new LinkedList<JobProcessNode>();
        // 流程任务
        ...
        task.setProcessInfo(TrainWorkPlan.getStatusMeaning(workPlan.getWorkPlanStatus()));
        ...
        for (JobProcessNode node : list) {
            ...
            if (node.getIsLeaf() == JobProcessNodeDef.CONST_INT_IS_LEAF_NO)                
                percentComplete = calPercentCompleteByList(jobProcessNodeQueryManager.getChildNodeList(node.getIdx()), node.getStatus());
            else
                percentComplete = node.getStatus().equals(JobProcessNode.STATUS_COMPLETE) ? 100 : 0;
            ...
            task.setProcessInfo(JobProcessNode.getStatusMeaning(node.getStatus()));
...
//            String planMode = JobProcessNode.PLANMODE_AUTO.equals(node.getPlanMode()) ? "自动" : "手动";
            ...
            if (node.getIsLeaf() != null && node.getIsLeaf() == JobProcessNodeDef.CONST_INT_IS_LEAF_NO) {
                ...
                // 查询下级节点工位是否分配
                task.setIsWsComplete(isWsComplete(node.getIdx()));
            }
            else {
     ...
     * @return
     */
    private int isWsComplete(String nodeIdx){
        // 默认完成，如果遍历下级所有叶子节点，发现存在未分配工位的，则返回0
        int result = 1 ;
        List<JobProcessNode> childList = jobProcessNodeQueryManager.getAllChildLeafNodeExceptThisList(nodeIdx);
        for (JobProcessNode node : childList) {
            if (node.getIsLeaf() != null && node.getIsLeaf() == JobProcessNodeDef.CONST_INT_IS_LEAF_YES){
    ...
    public int nextNodeList(String parentIDX, 
                            ...
                            int i, 
                            TrainWorkPlan workPlan,
                            Integer durationFormat, 
                            Integer milestone, 
                            List<Task> taskList, 
                            List<JobProcessNode> nodeList) throws Exception{
        List<JobProcessNode> nextNodeList = getNodeListByWorkPlanForGantt(workPlan.getIdx(), parentIDX);
        if (nextNodeList != null && nextNodeList.size() > 0) {
            ...
            for (JobProcessNode node : nextNodeList) {
                ...
                if (node.getIsLeaf() == JobProcessNodeDef.CONST_INT_IS_LEAF_NO)                
                    percentComplete1 = calPercentCompleteByList(jobProcessNodeQueryManager.getChildNodeList(node.getIdx()), node.getStatus());
                else
                    percentComplete1 = node.getStatus().equals(JobProcessNode.STATUS_COMPLETE) ? 100 : 0;
                if (node.getRatedWorkMinutes() != null) {
                    Double ratedWorkMinutes = node.getRatedWorkMinutes() / 60;
                    workDate = new BigDecimal(ratedWorkMinutes.toString()).setScale(2, BigDecimal.ROUND_HALF_UP);
                } 
...
//                String planMode = JobProcessNode.PLANMODE_AUTO.equals(node.getPlanMode()) ? "自动" : "手动";
     ...
     */
    @SuppressWarnings("unchecked")
    public Integer calPercentCompleteByList(List<JobProcessNode> list) {
     ...
     */
    @SuppressWarnings("unchecked")
    public Integer calPercentCompleteByList(List<JobProcessNode> list, String status) {
        Integer percentComplete = 0;
        if (list == null || list.size() < 1)
            return status.equals(JobProcessNode.STATUS_COMPLETE) ? 100 : 0;
        int count = 0;
        for (JobProcessNode node : list) {
            if (JobProcessNode.STATUS_COMPLETE.equals(node.getStatus()))
                count++;
     ...
     * @return 任务的前置任务数组
     */
    public Task[] addPredecessorLinks(List<JobProcessNode> nodeList, Task[] tasks, String workPlanIDX) {
        if (nodeList == null || nodeList.size() < 1)
            return tasks;
        for (JobProcessNode node : nodeList) {
            List<JobProcessNode> preNodeCaseList = jobProcessNodeQueryManager.getPreNodeCaseList(node.getIdx(), workPlanIDX);
     ...
     * @return 任务的前置任务序号
     */
    public Task[] addPredecessorLinkStr(List<JobProcessNode> nodeList, Task[] tasks, String workPlanIDX) {
        if (nodeList == null || nodeList.size() < 1)
            return tasks;
        for (JobProcessNode node : nodeList) {
            List<JobProcessNode> preNodeCaseList = jobProcessNodeQueryManager.getPreNodeCaseList(node.getIdx(), workPlanIDX);
     ...
     * @return 前置任务数组
     */
    public PredecessorLink[] getPredecessorLinks(List<JobProcessNode> nodeList, Task[] tasks) {
        List<PredecessorLink> list = new ArrayList<PredecessorLink>();
        if (nodeList == null || nodeList.size() < 1)
            return null;
        for (JobProcessNode nodeCase : nodeList) {
            for (Task task : tasks) {
     ...
     * @return 前置任务序号
     */
    public String getPrecessorLinkStr(List<JobProcessNode> nodeList, Task[] tasks) {
     ...
     */
    @SuppressWarnings("unchecked")
    private List<JobProcessNode> getNodeListByWorkPlanForGantt(String workPlanIDX, String parentIDX) throws Exception {
        StringBuffer hql = new StringBuffer("from JobProcessNode where recordStatus = 0 and workPlanIDX = '").append(workPlanIDX).append("'");
    ...
    private boolean getTaskExpanded(String nodeIdx) {
        if (!StringUtil.isNullOrBlank(nodeIDX)) {
            ...
            List<JobProcessNode> allList = new ArrayList<JobProcessNode>();
            allList.addAll(childList);
            allList.addAll(parentList);
            for (JobProcessNode node2 : allList) {
