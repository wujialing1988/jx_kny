 ...
 * @version 1.0
 */
public class UntreatedJobForYsyqr extends UnTreatedJobPermissionManager<TodoJob, TodoJob> implements IUntreatedJob {

	@Override
	public TodoJob getJob(String operatorid) {
		if (!super.checkPermission(Long.parseLong(operatorid), FUNC_YSYQR_NAME)) {
			return null;
		}
		int count = getCount();
		if (count <= 0) {
		...
		tdj.setJobType(FUNC_YSYQR_NAME);
	...
	private int getCount() {
		...
		return repairTaskListManager.taskListForAcceptor(searchEntity).getTotal();
	}
