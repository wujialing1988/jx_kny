 ...
 * @version 3.2.1
 */
public class UntreatedJobForTpDdpg extends UnTreatedJobPermissionManager <TodoJob,TodoJob> implements IUntreatedJob {
    ...
    public TodoJob getJob(String operatorid) {
        if(!checkPermission(Long.parseLong(operatorid), FUNC_TPDDPG_NAME))
            return null;
        int count = getCount();
        if(count <= 0)
            return null;
        TodoJob tdj = new TodoJob();
        tdj.setJobType(FUNC_TPDDPG_NAME);
        tdj.setJobText("未派工(" + count + ")");
        tdj.setJobUrl("/jsp/jx/jxgc/tpmanage/FaultTicketDdpg.jsp");
        tdj.setJobNum(count + "");
        return tdj;
    }
    ...
    private int getCount(){
        ...
        int count = 0;
        try {
            count = faultTicketManager.queryDdpgCount("false");
        } catch (Exception e) {
