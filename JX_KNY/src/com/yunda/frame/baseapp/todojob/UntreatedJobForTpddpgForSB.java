 ...
 * @version 3.0.1
 */
public class UntreatedJobForTpddpgForSB extends UnTreatedJobPermissionManager<TodoJob, TodoJob> implements IUntreatedJob {
	...
	public TodoJob getJob(String operatorid) {
		if (!super.checkPermission(Long.parseLong(operatorid), FUNC_TPDDPG_NAME2)) {
			return null;
		}
		int count = getCount();
		if (count <= 0) {
		...
		tdj.setJobType(FUNC_TPDDPG_NAME2);
	...
	private int getCount() {
		...
		return faultOrderManager.getDdpgCount();
	}
