 ...
 * @version 3.2.1
 */
public class UntreatedJobForTpcl extends UnTreatedJobPermissionManager<TodoJob,TodoJob> implements IUntreatedJob {
    ...
    public TodoJob getJob(String operatorid) {        
        ...
        tdj.setJobType(FUNC_TPCL_NAME);
        tdj.setJobText("未处理(" + count + ")");
        tdj.setJobUrl("/jsp/jx/jxgc/tpmanage/FaultTicketHandle.jsp");
        tdj.setJobNum(count + "");
        return tdj;
    }
    ...
    private int getCount(long operatorid){
        ...
        int count = 0;
        try {
            count = faultTicketManager.queryHandleCount(operatorid);
        } catch (Exception e) {
