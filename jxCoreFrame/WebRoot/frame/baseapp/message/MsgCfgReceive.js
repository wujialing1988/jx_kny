Ext.onReady(function(){
	...
	MsgCfgFunction.grid.getSelectionModel().on('rowselect', function(sm, rowIndex, record){
	...
	MsgCfgReceive.grid = new Ext.yunda.Grid({
		...
		fields: [{
	        ...
	        header:'规则标志', dataIndex:'ruleFlag', editor:{ maxLength:50 }        
		}],
		// 点击新增按钮触发事件前调用的方法。
	    ...
	    afterShowEditWin: function(record, rowIndex){
	    	// 回显"选择部门"字段值
	    	this.saveForm.find('xtype', 'OmOrganizationCustom_comboTree')[0].setDisplayValue('OmOrganizationCustom_comboTree_Id', record.get('receiverName'));
	    }
	});
	//在store.load前，组装查询参数
